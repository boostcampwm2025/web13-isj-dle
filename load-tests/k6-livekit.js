...
/*
================================================================================
LiveKit Cloud 직접 부하테스트 가이드
================================================================================

LiveKit Cloud의 미디어 서버 자체를 테스트하려면 공식 CLI 도구 사용:
...
3. 매개변수 설명:
   - --publishers: 비디오/오디오 송출자 수
   - --subscribers: 수신만 하는 참가자 수
   - --duration: 테스트 지속 시간
   - --video-resolution: 비디오 해상도 (360p, 720p, 1080p)
   - --video-codec: 코덱 (h264, vp8, vp9, av1)

4. 권장 테스트 시나리오:

   a) 일반 회의 (10명):
      lk load-test --publishers 10 --subscribers 0 --duration 60s

   b) 세미나 (1 발표자 + 199 청중):
      lk load-test --publishers 1 --subscribers 199 --duration 300s

   c) 최대 부하:
      lk load-test --publishers 50 --subscribers 150 --duration 300s

5. LiveKit Cloud 플랜별 제한:
   | 플랜      | 동시 참가자 | 월 연결 시간 |
   |-----------|------------|-------------|
   | Build     | 100명      | 5,000분     |
   | Ship      | 1,000명    | 150,000분   |
   | Scale     | 무제한     | 1,500,000분 |

   200명 동시 접속 목표 시 Ship 플랜 이상 필요!
